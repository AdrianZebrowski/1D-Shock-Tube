def vanleerMUSCL(test,c_max,dx):
    import numpy as np
    ...
    while t <= t_final:
        for i in range(0,ix): # Loop for calculating Fn 
            ...
            rhoL,rhoR = MUSCL(rho[im],rho[i],rho[ip],rho[ip2])
            aL,aR = MUSCL(a[im],a[i],a[ip],a[ip2])
            ML,MR = MUSCL(M[im],M[i],M[ip],M[ip2])
            
            F_plus[0,i] = ((1/4)*rhoL*aL*(1+ML)**2)
            F_plus[1,i] = ((1/4)*rhoL*aL*(1+ML)**2)*(2*aL/gamma)*(((gamma-1)/2)*ML+1)
            F_plus[2,i] = ((1/4)*rhoL*aL*(1+ML)**2)*(2*aL**2/(gamma**2-1))*(((gamma-1)/2)*ML+1)**2
            
            F_minus[0,i] = -((1/4)*rhoR*aR*(1-MR)**2)
            F_minus[1,i] = -((1/4)*rhoR*aR*(1-MR)**2)*(2*aR/gamma)*(((gamma-1)/2)*MR-1)
            F_minus[2,i] = -((1/4)*rhoR*aR*(1-MR)**2)*(2*aR**2/(gamma**2-1))*(((gamma-1)/2)*MR-1)**2
